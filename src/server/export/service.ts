...
import { assertCanExport, assertCanSelfExport } from "../authz/export";
...
import { describeFilters, normalizeExportFilters, type ExportFilters } from "./filters";
import { cellText } from "@/lib/officeverse/export/format";
import { EXPORT_QUERY, queryFollowUps, queryLeads } from "./queries";
import { buildXlsx } from "./xlsx";
import { getCloserByUserId } from "../db/repos/staff";
...
/* --------------------------- self-service export ------------------------- *
 * Admin UAT §12: a CLOSER may export THEIR OWN leads / follow-ups; ADMIN + HR
 * may self-export the full set. AGENTS may NOT export leads or follow-ups.
 * The scope is derived from the session — the caller cannot widen it. Reuses
 * the same bounded `queryLeads` / `queryFollowUps` + XLSX machinery.          */

export async function runMyExport(
  user: Pick<User, "id" | "role">,
  input: { dataset: "leads" | "followups"; format: "xlsx" | "csv" },
  meta: Meta = {},
): Promise<ExportFile> {
  assertCanSelfExport(user.role);
  const ds = input.dataset === "followups" ? EXPORT_DATASETS.followups : EXPORT_DATASETS.leads;

  const f: ExportFilters = {};
  if (user.role === "closer") {
    const c = await getCloserByUserId(user.id);
    if (!c) throw new HttpError(409, "Your user has no closer profile", "no_closer_profile");
    f.closerCode = c.closerCode; // own leads / own follow-ups only
  }
  // admin / hr → no scope filter (full dataset, same as the Admin export centre)

  const res = input.dataset === "followups" ? await queryFollowUps(f) : await queryLeads(f);
  if (res.capped) {
    throw new HttpError(
      413,
      `Your ${input.dataset} exceed ${MAX_EXPORT_ROWS.toLocaleString()} rows for a single export.`,
      "too_many_rows",
    );
  }

  const stamp = nowIST().slice(0, 10);
  const fileBase = `officeverse-my-${input.dataset}-${stamp}`;
