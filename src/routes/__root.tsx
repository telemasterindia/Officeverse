...
import { SessionProvider } from "../lib/officeverse/session";
    ...
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="max-w-md text-center">
        ...
        <h2 className="mt-4 text-xl font-semibold text-foreground">
          This corner of the TeleMaster India is empty
        </h2>
...
function ErrorComponent({ error, reset }: { error: Error; reset: () => void }) {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="max-w-md text-center">
        <div className="mx-auto grid h-16 w-16 place-items-center rounded-2xl bg-warning/12 text-3xl">
          🛠️
        </div>
        <h1 className="mt-4 font-display text-xl font-bold tracking-tight text-foreground">
          Something went wrong.
        </h1>
        <p className="mt-2 text-sm text-muted-foreground">Your data is safe. Please try again.</p>
...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    meta: [
      ...
      { title: "TeleMaster India — Your work. Your leads. Your wins." },
      {
        name: "description",
        content:
          "TeleMaster India is the internal sales and operations workspace for Exclusive Verified Leads — leads, follow-ups, teams and people in one hub.",
      },
